...
pub use handlers::{
    create_enviroments, create_feature_flag, create_org, get_envs, get_org, me, signin, signup,
...
async fn app(pool: Pool<Postgres>) -> Result<Router, Box<dyn std::error::Error>> {
    ...
    Ok(Router::new()
        ...
        .route("/orgs", post(create_org).get(get_org).patch(update_org))
        ...
        .route("/orgs/feature-flags/:flag_id", get(get_flags))
        .route("/me", get(me))
